<div align="center">
  <h3>🛠️ Projetos em Destaque</h3>
  
  <table border="0" width="100%">
    <tr>
      <td width="33%" align="center" valign="top">
        <a href="https://github.com/lgns-cin/CineMind">
          <img src=assets/CineMind.gif width="100%" style="border-radius: 15px;" alt="CineMind" />
        </a>
        <br>
        <br>
        <strong>🎬 CineMind</strong>
        <br>
        <sub>Sistema de recomendação e catálogo de filmes inteligente.</sub>
      </td>

      <td width="33%" align="center" valign="top">
        <a href="https://exoplanet-lia.streamlit.app/">
          <img src=assets/Nasa.gif" width="100%" style="border-radius: 15px;" alt="Nasa Exoplanet" />
        </a>
        <br>
        <br>
        <strong>🪐 NASA Exoplanet LIA</strong>
        <br>
        <sub>Aplicação Streamlit para análise de dados de exoplanetas.</sub>
      </td>

      <td width="33%" align="center" valign="top">
        <a href="https://github.com/AmandaTrinity/ProjetoIP">
          <img src=assets/ProjetoIP.gif" width="100%" style="border-radius: 15px;" alt="Projeto IP" />
        </a>
        <br>
        <br>
        <strong>💻 Projeto IP</strong>
        <br>
        <sub>Projeto final desenvolvido para a cadeira de Introdução à Programação.</sub>
      </td>
    </tr>
  </table>
</div>
